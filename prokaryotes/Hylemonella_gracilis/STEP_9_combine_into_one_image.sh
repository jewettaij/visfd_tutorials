...
# We will proceed in reverse-order.


# ----- Start with the ribosomes. -----
# The voxels in the "ribosomes.rec" file have brightness 1 if they
# are inside one of the ribosomes, and 0 otherwise
# We want to set the brightness of voxels in the ribosomes to 6.
...
# the voxels in the ribosomes, we want to invert the brightnesses of the
# voxels in the "ribosome_blobs.rec" file (swapping 1s with 0s).
...
# ----- Now include the nucleoid in the segmentation. -----
# Set the brightness of voxels in the nucleoid to 5.
...
# Add the voxels in "nucleoid.rec" to the voxels in "segmented_image.rec".
...
# So we remove the voxels in "nucleoid.rec" by subtracting their
...
combine_mrc unused_voxels.rec "-" nucleoid.rec unused_voxels.rec,0,1



# ----- Now include the storage granules in the segmentation (if present). -----
