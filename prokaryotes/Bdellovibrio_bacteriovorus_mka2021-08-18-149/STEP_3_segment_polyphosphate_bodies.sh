...
#    measured manually using a program like IMOD/3dmod and clicking on the image
...
#
# In this file, we use method #2.  The other methods are discussed
# in slightly more detail at the end of this file.
#
# 
# Using method #2 (automatic sherical blob-detection).
#
...
# standard blob detector algorithm to segment them automatically.
...
# comfortably encloses this range of diameters (eg 400.0 to 1400.0 angstroms).

filter_mrc -in orig_crop.rec -w 18.08 \
  -mask cytoplasm.rec \
  -blob minima ppb_blob_candidates.txt 400.0 1400.0 1.01
...
# If everything looks good, then replace the "ppb_blobs_annotated.rec" file with
# a new file ("ppb_blobs.rec") where the blobs are white on a black background.
